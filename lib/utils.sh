...
show_help() {
    ...
    echo "  $0 -i ~/Videos/Raw -p auto -m crf                           # Batch process directory with auto profiles"
    echo ""
    echo "📚 Manual Profile Selection:"
    ...
    echo "  $0 -i ~/Videos/Raw -p 1080p_anime -m abr                     # Process all videos in directory"
    echo "  $0 -i ~/Videos/Raw -p 4k_film -m crf                        # Batch process with CRF mode"
    echo "  $0 -i ~/Videos/Raw -p 1080p_film -m cbr --denoise            # Batch with denoising"
    echo ""
}
